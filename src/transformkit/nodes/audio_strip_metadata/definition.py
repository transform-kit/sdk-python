"""audio.strip-metadata node definition.

Strip ID3/container/tag metadata from an audio buffer via lossless stream copy.
Passes through unchanged when ``enabled`` is ``False``. Otherwise asks the
transport to re-emit the current buffer with metadata stripped using
...
audio_strip_metadata = NodeDefinition(type="audio.strip-metadata", execute=_execute)
